// Package db holds the PostgreSQL infrastructure every model of this process
// shares: the instrumented connection pool and the context-bound transaction.
// The repositories of the tenant and relation models build on it, so a write
// that spans both models (the owner membership of ClaimTenantOwnership, the
// permission check that guards a membership write) commits as one.
package db
