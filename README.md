...
1 回の再取得は最大 3 回まで試行し、接続失敗・タイムアウト・5xx・429 のときだけ 100ms、200ms 待って再試行する（それ以外の応答、JSON の不正、鍵の変換失敗は再試行せずに失敗させる）。
すべての試行が失敗した場合は 5 秒間のクールダウンを設け、その間の再取得は試みずに直前のエラーで失敗させる（キャッシュが TTL 内で `kid` が既知であれば、クールダウン中も検証は通る）。
検証鍵を解決できずに終わった場合（JWKS の取得失敗、クールダウン中）は `unavailable` を返し、未知の `kid`・署名の不正・期限切れのようなトークン側の不備は `unauthenticated` を返す。
